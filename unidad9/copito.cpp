...
int main() {
    ...
    for(int i = 0; i < casos; i++) {
        ...
        for(int d = 0; d< nodos; d++){//first find a leaf to know its parent, therefore y(middle node)
            if(adj[d].size() == 1){
                y = adj[adj[d][0]].size() - 1;//using the leaf as index, the size of its parent it's the connections 
                break;//of the middle node, so its fundamental to substract 1 since the root doesnt count for y
            }
        }
        x = edges / (y + 1);
